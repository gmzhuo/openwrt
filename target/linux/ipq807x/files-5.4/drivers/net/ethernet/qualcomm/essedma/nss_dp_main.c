 ...
 * nss_dp_get_stats64()
 */
void nss_dp_get_stats64(struct net_device *netdev,
...
#if 0//defined(NSS_DP_PPE_SUPPORT)
				   ...
				   struct gmac_hal_platform_data *hal_pdata)
{
	...
	printk("to deal macaddress \r\n");
	maddr = (uint8_t *)of_get_mac_address(np);
	printk("mac goted to check %p\r\n", maddr);
	if ((!IS_ERR_OR_NULL(maddr)) && !is_valid_ether_addr(maddr)) {
		printk("copy macaddr \r\n");
		ether_addr_copy(netdev->dev_addr, maddr);
	} else {
		printk("random macaddr \r\n");
		random_ether_addr(netdev->dev_addr);
		printk("to show macaddr\r\n");
